...
RUN apt-get update --fix-missing && \
    apt-get install -y wget bzip2 libgl1-mesa-glx ca-certificates curl git && \
    apt-get clean && \
...
RUN python3 -m pip install --no-cache-dir uvicorn==0.12.2 gunicorn==20.0.4 fastapi==0.61 pydantic==1.7.2 uvloop==0.14.0 websockets==8.1 httptools==0.1.1 python-multipart==0.0.5 aiofiles==0.6.0 sqlalchemy==1.3 databases[sqlite,postgresql]==0.4.1 psutil==5.7.3 alembic==1.4.3 redis==3.5.3 celery[redis]==5.0.2 psycopg2==2.9.2
COPY ./resources/start.sh /start.sh
COPY ./resources/ge.traineddata /opt/conda/share/tess/ge.traineddata
